...
class ClienteService:
  ...
  def insert(self, cliente,tipo_negociacao, rota, tabela_preco):
    ...
    # Alterar TAB PREÇO 
    data['RELACIONAMENTOS'] = {'RELACIONAMENTO': None if tabela_preco is None else tabela_preco.getIdTabelaPrecoVe()}
    ...
    data['COMERCIAL_VENDA'] = {'REPRESENTANTE': None if rota is None else rota.getIdRotaVe(), 
                               'MEIO_PAGAMENTO': 1 if tipo_negociacao is None else tipo_negociacao.getIdFormaPagamentoVe(), 
                               'CONDICAO_PAGAMENTO': 1 if tipo_negociacao is None else tipo_negociacao.getIdCondicaoPagamentoVe(), 
                               'SITUACAO_CADASTRO': 1 if cliente.getAtivo() == 'S' else 3, 
                               'PERFIL': 1, 
                               'PERFIL_RESTRICAO': 1 if cliente.getBloquear() == 'N' else 2 }
    data['ENDERECOS']       = [{'TIPO': 'R', 
                                ...
                                'CIDADE': {'NOME': cliente.getCidade()}, 
                                'ESTADO': {'SIGLA': cliente.getEstado()}, 
                                'PAIS': {'NOME': 'BRASIL'}, 
                                'CAIXA_POSTAL': '', 
                                'DESCRICAO': '', 
                                'CONTATO_ALTERNATIVO': '',
                                'POSICIONAMENTO_LATITUDE': cliente.getLatitude(), 
                                'POSICIONAMENTO_LONGITUDE': cliente.getLongitude()
                              }]
  ...
  def update(self, cliente, tipo_negociacao, rota, tabela_preco):
    headers = {'Content-type': 'application/json', 'Accept': 'text/plain'}
    ...
    data['COMERCIAL_VENDA'] = {'REPRESENTANTE': None if rota is None else rota.getIdRotaVe(), 
                               ...
                               'CONDICAO_PAGAMENTO': 1 if tipo_negociacao is None else tipo_negociacao.getIdCondicaoPagamentoVe(), 
                               'SITUACAO_CADASTRO': 1 if cliente.getAtivo() == 'S' else 3, 
                               'PERFIL': 1 if cliente.getBloquear() == 'N' else 2, 
                               'PERFIL_RESTRICAO': 1 }
    data['ENDERECOS']       = [{'TIPO': 'R', 
                                'PRINCIPAL': 'S', 
                                'CEP': cliente.getCep(), 
